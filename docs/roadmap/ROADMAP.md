...
- [x] **Diff Viewer** ⭐ — avant/après côte à côte, surlignage des modifications *(2026-09-05 : diff hiérarchique lignes puis mots - plus aucune impasse « texte trop long », un CV de n'importe quelle taille a son Avant/Après ; premiers tests frontend (Vitest) sur cet organe et sur les filets d'erreur)*
...
- [x] **Confirmation Avant/Après reliée à la banque de preuves** *(ajout 2026-07-28, retour test réel — concrétise la promesse déjà affichée « Bientôt, chaque confirmation se reliera à ta banque de preuves » — **complet 2026-08-12**)* — à côté de chaque bouton « Confirmer — vrai et prouvable », un champ **optionnel** « Comment ? / écris ici ta preuve » (ex. « Chez X, j'ai mis en place le pipeline CI/CD : build, lint, test »). Au clic « Confirmé », si le champ est rempli → **créer une preuve** (type note) dans Profil & Preuves. Boucle vertueuse : l'ajout au CV devient une preuve réutilisable, et l'utilisateur muscle sa banque sans quitter son parcours. Ne jamais forcer : vide = simple confirmation, comme aujourd'hui.
  - **Écart assumé (2026-08-12)** : la promesse initiale disait « reliée **au fait correspondant** ». Or l'Avant/Après compare du **texte brut** — il n'existe aucun fait à relier à ce stade (le CV n'est pas encore un modèle structuré). La preuve est donc créée **autonome** : titre = le passage ajouté (le fait à prouver), contenu = le « Comment ? ». Le rattachement à un fait existant deviendra naturel avec le **CV semi-structuré (V2)**, où chaque passage sera déjà un fait. En attendant, la boucle vertueuse fonctionne : la preuve rejoint la banque, prête à resservir.
  - **Détails d'implémentation** : creation via l'endpoint `POST /proofs` existant (`type: note`, confidentialité `private` par défaut — cohérent pour un extrait de CV perso). Jamais bloquant : si le backend est absent, l'ajout reste confirmé et un avertissement invite à re-confirmer. Idempotent par ajout (pas de doublon si on bascule off/on). Champ verrouillé une fois la preuve enregistrée.
- [x] **Matching plus tolérant — mots voisins / lemmatisation légère** *(ajout 2026-07-28, retour test réel ; **livré 2026-09-05**)* — aujourd'hui « déployer » (mot-clé de l'offre) et « déploiement » (écrit dans le CV) ne se rapprochent pas : le stemming minimal ne réunit pas verbe et nom d'une même racine, ni les variantes orthographiques (déploie-/déploy-). Piste : lemmatisation légère FR ou table de familles de mots, **sans** sur-généraliser (le sur-matching donnerait un faux score de couverture — pire que l'inverse). À valider sur des offres réelles ; le mode IA optionnel pourrait aussi couvrir ce besoin plus tard. **Statut : à confirmer.**
  - **Livré (2026-09-05)** : stemmer léger dans `keyword_engine.py` (miroir exact côté TS pour le surlignage live). Un seul suffixe dérivationnel retiré parmi `-ement / -ation / -ateur / -eur / -er / -e`, uniquement s'il reste un radical d'au moins 5 lettres, plus l'alternance y/i (déploy-/déploi-) et les pluriels en -x. Familles réunies : déploiement / déployer / déployé, configuration / configurer, administrateur / administration, utilisateur / utiliser, analyse / analyser. **Volontairement absents** : `-tion` seul (gestion > ges), `-ant` (important / importer), `-ir`, `-ité`, `-ique` - le sur-matching reste l'ennemi. Tests de familles ET de non-collision (forme / formation, geste / gestion) des deux côtés.
- [ ] **« Exporter mon retour d'expérience » — mesurer sans télémétrie** *(ajout 2026-09-05)* — la V1 promet de mesurer le MVP statement, mais zéro télémétrie + SQLite local = aucun canal de collecte ; demander le ZIP de sauvegarde à un testeur, c'est lui demander son CV entier. Proposition : un bouton qui produit un **fichier anonyme** (scores avant/après, temps passé par étape, niveau d'IA utilisé, réponses du micro-suivi) **sans aucun contenu de CV ni d'offre**, que le testeur relit à l'écran avant de l'envoyer lui-même par mail. Rien ne part tout seul, il voit exactement quoi. Cohérent avec « CVForge n'envoie jamais rien » et rend le MVP statement mesurable au lieu de rester un slogan.
...
- [x] **Convention anti-tells IA — outillage** *(ajout 2026-07-12 ; règle complète dans CLAUDE.md — **complet 2026-08-12**)* — aucun caractère « signature IA » (tirets cadratins, ellipse unique, quotes courbes, flèches) dans les textes visibles ; les accents français ne sont jamais touchés par le lint. *(fait 2026-07-12 : purge des sources frontend + backend, script `npm run lint:tells`)*. *(fait 2026-08-12 : **hook pre-commit** `.githooks/pre-commit` qui bloque le commit via `check-tells.mjs` — activé par `core.hooksPath`, contournable en dernier recours par `--no-verify` ; **règle ESLint dédiée** `no-restricted-syntax` sur littéraux + templates TS, message pédagogique, accents jamais visés ; `check-tells.mjs` scanne désormais aussi `backend/app`)*
...
- [ ] **Packaging desktop — exe portable façon ADWCleaner** : PyInstaller `--onefile` (Python + FastAPI + build Angular dans un seul exécutable). Double-clic → serveur local → navigateur. **Données : résolues par `resolve_data_dir()` (câblée dès V1)** — défaut `~/.cvforge/` (mode installé, données préservées si on remplace l'exe) ; mode portable activé par un marqueur `cvforge.portable` à côté de l'exe → données dans `./data/` qui voyagent sur la clé USB. Le même binaire fait les deux. Option fenêtre native : pywebview (WebView2). Signature de code à prévoir contre SmartScreen (Azure Trusted Signing ~10 $/mois). Pas d'UPX (faux positifs antivirus). Tauri abandonné — plus nécessaire.
  - **Alternative au `--onefile` — dossier portable zippé (réf. Dolphin Emulator, 2026-07-28)** : Dolphin distribue un **dossier déjà décompressé** (exe + DLL côte à côte) plutôt qu'un exe auto-extractible unique. Avantage : **zéro délai de décompression** à chaque lancement (le compromis du `--onefile`). Inconvénient : un geste « extraire le ZIP » en plus (friction pour un public peu à l'aise). **À garder en réserve** : si le `--onefile` s'avère trop lent au démarrage en usage réel, livrer un dossier portable zippé.
  - **Confiance au téléchargement = réputation, pas format** (confirmé par l'exemple Dolphin, 20 ans d'historique) : un ZIP/exe n'inspire pas confiance par son format. CVForge, sans historique, remplace la réputation par des **preuves techniques** (VirusTotal, signature de code, code source public) en attendant qu'elle se construise. Renforce la stratégie de diffusion déjà actée.
  - **Fenêtre native pywebview — fait 2026-08-12 (repli garanti)** : `desktop.py` ouvre une vraie fenêtre d'application (pywebview / WebView2) ; fermer la fenêtre arrête proprement le serveur (uvicorn dans un thread démon, signaux neutralisés). **Dégradation sans faille** : si pywebview est absent, si WebView2 manque, ou si la création de fenêtre échoue → repli automatique sur le navigateur (comportement historique prouvé). Échappatoires : `CVFORGE_NO_WINDOW=1` force le navigateur, `CVFORGE_NO_BROWSER=1` ne lance ni fenêtre ni navigateur (serveur/tests). pywebview = dépendance **optionnelle** (`.[desktop]`), embarquée par le spec via `collect_all("webview")` de façon non bloquante, installée par `build_portable.py`.
    - **Console noire supprimée (build fenêtré, 2026-08-12)** : première tentative = masquage à chaud (`ctypes ShowWindow`), **inefficace sur Windows 11** (Windows Terminal ne répond pas comme l'ancien `conhost`) - confirmé par retour test réel (double-clic → la console restait). Solution retenue : geler l'exe en **mode fenêtré** (`console=False`) → aucune console créée. Compromis gérés : (1) les erreurs de démarrage passent par une **boite de dialogue Windows** (`MessageBoxW`) au lieu de la console ; (2) en repli navigateur, une boite « ferme pour arreter » joue le rôle de bouton d'arrêt ; (3) `sys.stdout/stderr` garantis non-`None` avant uvicorn (sinon plantage en mode fenêtré). Le chemin `CVFORGE_NO_BROWSER` reste un `uvicorn.run` bloquant classique.
    - **Vérifié** : build OK (40 Mo), serveur gelé démarre + migrations Alembic + SPA/API servies en headless ; fenêtre native confirmée à l'écran par l'utilisateur (2026-08-12). **Reste à valider par l'utilisateur** : disparition de la console au **double-clic** (mode fenêtré), repli navigateur (`CVFORGE_NO_WINDOW=1`), et test sur machine vierge (WebView2 présent par défaut sur Windows 11 ; à confirmer sur Windows 10 plus ancien, sinon repli navigateur).
  - **Validation externe de la contrainte sync** : Dolphin PC et Dolphin Android sont deux installations **non synchronisées** — exactement le mur de l'app mobile CVForge (V3, dépend de la sync). Rien à changer, juste une confirmation de l'ordre déjà acté (sync d'abord, mobile ensuite).
...
- [ ] **Curseur d'effort — proposer une INTENSITÉ, pas un protocole unique** *(ajout 2026-07-28, tiré d'une préparation réelle poussée à l'extrême)* — le module de préparation offre trois niveaux, choisis par l'utilisateur selon l'enjeu réel du poste. Constat de fond : personne ne peut fournir un effort maximal sur 20 candidatures, mais **le même raisonnement à 20 % d'intensité produit déjà l'essentiel du bénéfice**.
  - **Rapide (~10 min)** — les 3 exigences principales de l'offre, une preuve pour chacune. Suffit pour une candidature d'opportunité. **C'est le mode par défaut.**
  - **Sérieux (~1 h)** — chaque exigence confrontée à une preuve, avec statut (vécu / documenté / hypothèse / manquant) + questions à poser.
  - **Poussé (~1 journée+)** — + recherche employeur, + fabriquer une preuve concrète sur un manque identifié, + script chronométré.
  - ⚠️ **Garde-fou, aussi important que la feature — savoir dire STOP** : un outil de préparation peut produire de l'anxiété (plus on prépare, plus on craint de gâcher). Retour vécu : à un entretien, le meilleur niveau technique de tous les candidats, mais le stress visible en fin d'échange a fait perdre le poste. Donc : **ne JAMAIS suggérer « tu pourrais préparer davantage »**, ne JAMAIS afficher de jauge de complétude qui pousse au 100 %. À l'inverse, **message de clôture explicite** quand le niveau choisi est atteint (« Ta préparation est suffisante pour cette candidature. »). Savoir s'arrêter est une fonctionnalité.
  - ⚠️ **Rappel de cadrage (anti-devin)** : ne PAS construire de feature qui **interprète** l'intention de l'offre (« ils disent X mais veulent sûrement Y »). Deviner le besoin caché est un réflexe humain utile, mais l'automatiser ferait de CVForge un devin - exactement le geste qu'il interdit côté CV. Rester sur ce qui est écrit ; laisser l'utilisateur formuler ses hypothèses comme des **questions à poser en entretien**, jamais comme des affirmations.
...
- [ ] **Détection « offre surchargée / non spécifique » — protéger le candidat** *(ajout 2026-07-28, retour test réel : offre « Développeur Full Stack IA » exigeant 6+ langages backend, 2 clouds, 3 BDD relationnelles + NoSQL, IA générative avancée, le tout sur une fiche, fourchette 32 065,43 € - 62 356,03 € — précision au centime sur une plage du simple au double)* — voisin des « signaux faibles » mais **orienté protection du candidat**, pas information sur l'entreprise : quand une offre est structurellement floue, un score de matching bas **n'est pas la faute du candidat**.
  - **Détection (heuristique, sans IA)** : compter les compétences/technologies distinctes extraites (déjà fait par le moteur de matching) ; au-delà d'un seuil (à calibrer, ex. > 20-25 disjointes pour un seul poste) → alerte. Signal aggravant optionnel : fourchette salariale très large (ratio max/min > 1.5) + précision inhabituelle (centimes) = fourchette non réfléchie.
  - **Message (jamais culpabilisant pour le candidat)** : « Cette offre liste un nombre inhabituel de compétences disjointes - signe fréquent d'un poste mal défini ou d'un copier-coller de template. Un score de couverture bas ici ne veut pas forcément dire que ton profil ne convient pas : la vraie question est peut-être : ce poste correspond-il à ce qu'ils cherchent réellement ? »
  - **Cohérence philosophie** : anti-hallucination à l'envers — au lieu de pousser à gonfler le CV pour matcher une offre absurde, CVForge dit la vérité sur **l'offre elle-même**.
