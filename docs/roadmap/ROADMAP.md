...
- [ ] **Packaging desktop — exe portable façon ADWCleaner** : PyInstaller `--onefile` (Python + FastAPI + build Angular dans un seul exécutable). Double-clic → serveur local → navigateur. **Données : résolues par `resolve_data_dir()` (câblée dès V1)** — défaut `~/.cvforge/` (mode installé, données préservées si on remplace l'exe) ; mode portable activé par un marqueur `cvforge.portable` à côté de l'exe → données dans `./data/` qui voyagent sur la clé USB. Le même binaire fait les deux. Option fenêtre native : pywebview (WebView2). Signature de code à prévoir contre SmartScreen (Azure Trusted Signing ~10 $/mois). Pas d'UPX (faux positifs antivirus). Tauri abandonné — plus nécessaire.
  ...
  - **Fenêtre native pywebview — fait 2026-08-12 (repli garanti)** : `desktop.py` ouvre une vraie fenêtre d'application (pywebview / WebView2) et **masque la console** (via `ctypes ShowWindow`) quand elle s'ouvre ; fermer la fenêtre arrête proprement le serveur (uvicorn dans un thread démon, signaux neutralisés). **Dégradation sans faille** : si pywebview est absent, si WebView2 manque, ou si la création de fenêtre échoue → repli automatique sur le navigateur (comportement historique prouvé), console ré-affichée. Échappatoires : `CVFORGE_NO_WINDOW=1` force le navigateur, `CVFORGE_NO_BROWSER=1` ne lance ni fenêtre ni navigateur (serveur/tests). pywebview = dépendance **optionnelle** (`.[desktop]`), embarquée par le spec via `collect_all("webview")` de façon non bloquante, installée par `build_portable.py`. Vérifié : build OK (40 Mo), serveur gelé démarre + migrations + SPA/API servies en headless. **Reste à valider par l'utilisateur (non testable sans écran)** : rendu visuel de la fenêtre + masquage console sur une vraie machine, et test sur machine vierge (WebView2 présent par défaut sur Windows 11 ; à confirmer sur Windows 10 plus ancien, sinon repli navigateur).
